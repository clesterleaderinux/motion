...
class MotionViewGridLayout @JvmOverloads constructor(
    context: Context,
    ...
    private fun initMotionView(
        ...
        val a = context.obtainStyledAttributes(attrs, R.styleable.MotionViewGridLayout, 0, 0)
        motionValues.also { motionView.motionValues = it }

        val motionProperties = a.getInteger(
            R.styleable.MotionViewGridLayout_motion_type_key,
            MotionTypeKey.None.index,
        )

        motionView.playTogether = a.getBoolean(
            R.styleable.MotionViewGridLayout_motion_playtogether, true,
        )
        motionView.duration = a.getInteger(
            R.styleable.MotionViewGridLayout_motion_duration,
            MotionDuration.DurationMedium01.speedInMillis.toInt(),
        ).toLong()
        motionView.motionState = a.getInt(
            R.styleable.MotionViewGridLayout_motion_transition,
            MotionState.Exiting.index,
        )
        motionView.motionKey = a.getString(
            R.styleable.MotionViewGridLayout_motion_key,
        )
        MotionInterpolator.values()[
            a.getInt(
                R.styleable.MotionViewGridLayout_motion_curve,
                MotionCurve.EasingEase01.index,
            ),
        ].interpolator.also { motionView.curveEnter = it }

        motionView.chainDelay = a.getInt(
            R.styleable.MotionViewGridLayout_motion_chain_delay,
            0,
        )
        motionView.chainIndex = a.getInt(
            R.styleable.MotionViewGridLayout_motion_chain_index,
            0,
        )
        motionView.chainKey = a.getString(
            R.styleable.MotionViewGridLayout_motion_chain_key,
        )

        val translationX = TranslationX(
            xEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_xEnter,
                0,
            ).toFloat(),
            xIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_xIn,
                0,
            ).toFloat(),
            xExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_xExit,
                0,
            ).toFloat(),
        )
        ...
        val translationY = TranslationY(
            yEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_yEnter,
                0,
            ).toFloat(),
            yIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_yIn,
                0,
            ).toFloat(),
            yExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_yExit,
                0,
            ).toFloat(),
        )
        ...
        val alpha = Alpha(
            aEnter = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_aEnter,
                1f,
            ),
            aIn = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_aIn,
                1f,
            ),
            aExit = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_aExit,
                1f,
            ),
        )
        ...
        val scale = Scale(
            sEnter = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_sEnter,
                1f,
            ),
            sIn = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_sIn,
                1f,
            ),
            sExit = a.getFloat(
                R.styleable.MotionViewGridLayout_motion_sExit,
                1f,
            ),
        )
        ...
        val scrollX = ScrollX(
            sEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_scrollXEnter,
                0,
            ).toFloat(),
            sIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_scrollXIn,
                0,
            ).toFloat(),
            sExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_scrollXExit,
                0,
            ).toFloat(),
        )
        ...
        val elevation = Elevation(
            eEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_eEnter,
                0,
            ).toFloat(),
            eIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_eIn,
                0,
            ).toFloat(),
            eExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_eExit,
                0,
            ).toFloat(),
        )
        val resize = Resize(
            hEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_hEnter,
                0,
            ).toFloat(),
            hIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_hIn,
                0,
            ).toFloat(),
            hExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_hExit,
                0,
            ).toFloat(),
            wEnter = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_wEnter,
                0,
            ).toFloat(),
            wIn = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_wIn,
                0,
            ).toFloat(),
            wExit = a.getDimensionPixelSize(
                R.styleable.MotionViewGridLayout_motion_wExit,
                0,
            ).toFloat(),
        )
        ...
        onEnterText = a.getString(
            R.styleable.MotionViewGridLayout_on_enter_text,
        )
        onInText = a.getString(
            R.styleable.MotionViewGridLayout_on_in_text,
        )
        onExitText = a.getString(
            R.styleable.MotionViewGridLayout_on_exit_text,
        )
        a.recycle()
    }
}